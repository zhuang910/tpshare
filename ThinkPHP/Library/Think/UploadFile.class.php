...
class UploadFile {
    ...
    private $config = array(
        ...
        'thumbPrefix'       => '', // 缩略图前缀
        'thumbSuffix'       => '_thumb', // 缩略图后缀
        'thumbMaxWidth'     => '', // 缩略图最大宽度 '100,50'
        'thumbMaxHeight'    => '', // 缩略图最大高度 '100,50'
        'thumbPath'         => '', // 缩略图保存路径
        'thumbFile'         => '', // 缩略图文件名
    );
        ...
        foreach ($files as $key => $file) {
            ...
            if($this->thumb) {
                ...
                if(false !== $image) {
                    ...
                    $thumbPath = $this->thumbPath?$this->thumbPath:($this->rootPath . $file['savepath']);
                    // 生成图像缩略图
                    import($this->imageClassPath);
                    $image = new Image($this->imageDriver);
                    $image->open($filename);
                    $realFilename  =  $this->autoSub?basename($file['name']):$file['name'];
                    
                    for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                        $thumbname	=	$thumbPath.$thumbPrefix[$i].$thumbFile[$i].$thumbSuffix[$i].'.'.$file['ext'];
                        $image->thumb($thumbWidth[$i],$thumbHeight[$i],6)->save($thumbname);
                    }
